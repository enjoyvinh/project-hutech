...
angular.module('exexTable', ['ui.bootstrap.pagination'])
  .controller('exexTableCtrl', ['$scope', '$attrs', 'orderByFilter', '$filter', '$parse',
    function($scope, $attrs, orderByFilter, $filter, $parse) {
      ...
      $scope.SelectedCol = function(column) {
        if(column == $scope.sort.column){
          return ('fa fa-sort-' + (($scope.sort.descending) ? 'desc' : 'asc'));
        }else{
          return 'fa fa-sort';
        }
  ...
  .directive('exexTable', ['$compile',
    function($compile) {
      return {
        scope: true,
        replace: true,
        transclude: true,
        //templateUrl: 'exexTable.html',
        templateUrl: 'exexTablePagination',
        controller: 'exexTableCtrl',
        link: function(scope, elm, attrs, controller, transcludeFn) {
          if ( !! scope.tableDataFiltered.length) {
            ...
            $compile(tbody)(scope, function(clone) {
              elm.addClass(attrs.exexTable).append(clone);
            });
          }
        }
      };
